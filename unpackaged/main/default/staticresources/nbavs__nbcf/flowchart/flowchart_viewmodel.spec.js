describe('flowchart-viewmodel', function () {
	...
	it('test can add output connector to node', function () {
		...
		expect(testObject.outputs.length).toBe(2);
		expect(testObject.outputs[0].data).toBe(data1);
		expect(testObject.outputs[1].data).toBe(data2);

		expect(testObject.data.outputs.length).toBe(2);
		expect(testObject.data.outputs[0]).toBe(data1);
		expect(testObject.data.outputs[1]).toBe(data2);
	});
	...
	it('data model with existing connection creates a connection view model', function () {
		...
		expect(testObject.connections[0].source.data).toBe(mockDataModel.nodes[0].outputs[0]);
	...
	it('test control + mouse down toggles connection selection', function () {
 	...
 	it('test data-model is wrapped in view-model', function () {
		...
		var mockOutputConnector = mockNode.outputs[0];
		...
		expect(node.outputs[0].data).toBe(mockOutputConnector);

		// Connection
 	
		expect(testObject.connections.length).toBe(1);
		expect(testObject.connections[0].source).toBe(testObject.nodes[0].outputs[0]);
	...
	it('test find output connector retreives correct connector', function () {
		...
		expect(testObject.findOutputConnector(15, 1)).toBe(testObject.nodes[2].outputs[1]);
	});
	...
	it('test create new connection', function () {
		...
		var startConnector = testObject.nodes[0].outputs[0];
	...
	it('test create new connection from input to output', function () {
		...
		var endConnector = testObject.nodes[0].outputs[0];
